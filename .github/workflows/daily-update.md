...
- **5. コミュニティイベント情報（東京・神奈川）および大手ベンダー・大規模カンファレンス** —
  `docs/events.json` の `events` から作成。`vendor_event` が付かないイベントはコミュニティイベントとして扱い、
  `vendor_event: true` は参加レポート/ベンダーカンファレンス関連として扱う。
...
（`vendor_event` が付かないイベントから 4〜6 件。次の形式で記載）
